...
import { createLearningPath } from '@/lib/pathApi';
...
export function PathBuilderForm({
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      /*
       * Save the learning path in PostgreSQL.
       *
       ...
       * goal
       */
      const savedPath = await createLearningPath({
        title: cleanGoal,
        description:
          `Learning path for ${experience} level. ` +
          `Purpose: ${purpose}. ` +
          `${hasDeadline ? `Target duration: ${days} days.` : 'Self-paced.'}`,
        goal: cleanGoal,
      });
      ...
      /*
       * Keep the existing frontend generation
